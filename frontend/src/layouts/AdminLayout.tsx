/**
 * AdminLayout — sidebar + topbar layout for admin role.
 *
 * Full nav: Dashboard, All Leaves, Approvals, Users, Leave Types, Reports
 */
...
export function AdminLayout() {
  ...
  const displayName = user?.name || (user?.first_name ? `${user.first_name} ${user.last_name}`.trim() : user?.email) || "Administrator";
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-16 items-center gap-4 border-b bg-card px-6">
          <button className="md:hidden" onClick={() => setSidebarOpen((v) => !v)} aria-label="Toggle sidebar">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="rounded-full bg-destructive/10 px-3 py-1 text-xs font-semibold text-destructive uppercase tracking-wider">
              {user?.role || "ADMIN"}
            </span>
