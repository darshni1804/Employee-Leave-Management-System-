/**
 * EmployeeLayout — sidebar + topbar layout for employee role.
 *
 * Sidebar links: Dashboard, My Leaves, Leave Balances, Profile
 */
...
const navLinks = [
  ...
  { to: "/balances", label: "Leave Balances", icon: Wallet },
...
export function EmployeeLayout() {
  ...
  const displayName = user?.name || (user?.first_name ? `${user.first_name} ${user.last_name}`.trim() : user?.email) || "Employee";
        ...
        <div className="border-t p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-primary text-sm font-semibold">
              {user?.first_name?.charAt(0) || user?.email?.charAt(0) || "E"}
